...
def main(
    ctx: Context,
    function_path: str = typer.Argument(..., help="Modal function path in format app_name.function_name"),
):
    ...
    if "." not in function_path:
        typer.echo(f"Error: Invalid format. Expected 'app_name.function_name', got '{function_path}'", err=True)
        raise typer.Exit(1)
